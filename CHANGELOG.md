...
- **Releases are signed, and `askr upgrade` refuses one that is not.** The trust chain
  ...
  **Two manual steps remain, and they are deliberately not automated.** The signing key
  must be generated by whoever holds it — `minisign -G -W -p keys/release.pub -s
  askr-release.key` — with the public half committed and the secret half in the
  repository secret `MINISIGN_SECRET_KEY`. A key generated anywhere it could be observed
  is compromised the moment it exists, so this one is not created for you. `docs/RELEASING.md`
  step 0 has the procedure, including the part worth reading twice: losing the key locks
  out every future release, because installs built against the old key will refuse the new
  tarballs.

  Until a real key is committed, `upgrade` behaves as before and says so on every run —
  an unconfigured build must not be a build that cannot upgrade. The verification path is
  tested against minisign's published prehashed test vector, and there is a test that
  fails if `keys/release.pub` ever contains that vector's *public* key, whose secret half
  is also published.
