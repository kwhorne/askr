...
| **Every** page answers **400** over HTTP/1.x, while the same URL over HTTP/2 answers 200 — including `/up`, and static files are fine | `HTTP_HOST` reached PHP as `host, host:port`: it was set from the effective host and then re-added from the `Host:` header, and 1.5.1's repeated-field join merged the two. Symfony reads a comma in the host as a `SuspiciousOperationException`, so `Request::create()` threw before the kernel ran. h2 carries the authority in the URI, so it had nothing to duplicate — which is why pinning a client to `2.0` "fixed" it. Static files never build `$_SERVER`, which is why `/favicon.ico` kept working. Fixed 1.5.2. |
